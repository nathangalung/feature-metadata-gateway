...
def test_create_feature_metadata_internal_error(monkeypatch):
    ...
    monkeypatch.setattr(feature_service, "create_feature_metadata", lambda x: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/create_feature_metadata", json={
...
def test_get_feature_metadata_post_success():
    ...
    client.post("/create_feature_metadata", json=data)
    resp = client.post("/get_feature_metadata", json={
        "feature_name": "main:getpost:v1",
        "user_role": "developer"
    })
    assert resp.status_code == 200
...
def test_get_feature_metadata_post_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "get_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/get_feature_metadata", json={
        "feature_name": "main:fail:v1",
        "user_role": "developer"
    })
...
def test_get_all_feature_metadata_post_success():
    ...
    client.post("/create_feature_metadata", json=data)
    resp = client.post("/get_all_feature_metadata", json={
        "user_role": "developer"
    })
    assert resp.status_code == 200
...
def test_get_all_feature_metadata_post_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "get_all_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/get_all_feature_metadata", json={
        "user_role": "developer"
    })
    assert resp.status_code == 400
...
def test_get_all_feature_metadata_post_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "get_all_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/get_all_feature_metadata", json={
        "user_role": "developer"
    })
    assert resp.status_code == 500
...
def test_get_deployed_features_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "get_deployed_features", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.get("/get_deployed_features?user_role=developer")
    assert resp.status_code == 500
...
def test_features_endpoint():
    resp = client.post("/features", json={"features": ["main:success:v1"], "entities": {}})
    assert resp.status_code == 200
...
def test_update_feature_metadata_success():
    ...
    client.post("/create_feature_metadata", json=data)
    resp = client.post("/update_feature_metadata", json={
        "feature_name": "main:update:v1",
        "last_updated_by": "dev",
        "user_role": "developer"
    })
...
def test_update_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "update_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/update_feature_metadata", json={
...
def test_update_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "update_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/update_feature_metadata", json={
        "feature_name": "main:updatefail:v1",
        "last_updated_by": "dev",
        "user_role": "developer"
...
def test_delete_feature_metadata_success():
    ...
    client.post("/create_feature_metadata", json=data)
    resp = client.post("/delete_feature_metadata", json={
        "feature_name": "main:delete:v1",
        "deleted_by": "dev",
        "user_role": "developer",
...
def test_delete_feature_metadata_deployed():
    ...
    client.post("/create_feature_metadata", json=data)
    # Deploy the feature
    client.post("/ready_test_feature_metadata", json={
        "feature_name": "main:deploydel:v1",
        "submitted_by": "dev",
        "user_role": "developer"
    })
    client.post("/test_feature_metadata", json={
        "feature_name": "main:deploydel:v1",
        "test_result": "TEST_SUCCEEDED",
        "tested_by": "test_system",
        "user_role": "external_testing_system"
    })
    client.post("/approve_feature_metadata", json={
        "feature_name": "main:deploydel:v1",
...
def test_delete_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "delete_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/delete_feature_metadata", json={
...
def test_delete_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "delete_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/delete_feature_metadata", json={
...
def test_ready_test_feature_metadata_success():
    ...
    client.post("/create_feature_metadata", json=data)
    resp = client.post("/ready_test_feature_metadata", json={
        "feature_name": "main:readytest:v1",
        "submitted_by": "dev",
        "user_role": "developer"
    })
...
def test_ready_test_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "ready_test_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/ready_test_feature_metadata", json={
...
def test_ready_test_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "ready_test_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/ready_test_feature_metadata", json={
        "feature_name": "main:readytestfail:v1",
        "submitted_by": "dev",
        "user_role": "developer"
...
def test_test_feature_metadata_success():
    ...
    resp = client.post("/test_feature_metadata", json={
        "feature_name": "main:testfeature:v1",
        "test_result": "TEST_SUCCEEDED",
        "tested_by": "test_system",
        "user_role": "external_testing_system"
...
def test_test_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "test_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/test_feature_metadata", json={
...
def test_test_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "test_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/test_feature_metadata", json={
        "feature_name": "main:testfeaturefail:v1",
        "test_result": "TEST_SUCCEEDED",
        "tested_by": "test_system",
...
def test_approve_feature_metadata_success():
    ...
    resp = client.post("/approve_feature_metadata", json={
        "feature_name": "main:approvefeature:v1",
        "approved_by": "approver",
        "user_role": "approver"
    })
...
def test_approve_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "approve_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/approve_feature_metadata", json={
...
def test_approve_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "approve_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/approve_feature_metadata", json={
        "feature_name": "main:approvefeaturefail:v1",
        "approved_by": "approver",
        "user_role": "approver"
...
def test_reject_feature_metadata_success():
    ...
    resp = client.post("/reject_feature_metadata", json={
        "feature_name": "main:rejectfeature:v1",
...
def test_reject_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "reject_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/reject_feature_metadata", json={
...
def test_reject_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "reject_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/reject_feature_metadata", json={
        "feature_name": "main:rejectfeaturefail:v1",
        "rejected_by": "approver",
        "user_role": "approver",
...
def test_fix_feature_metadata_success():
    ...
    resp = client.post("/fix_feature_metadata", json={
        "feature_name": "main:fixfeature:v1",
        "fixed_by": "dev",
        "user_role": "developer",
        "fix_description": "fix"
...
def test_fix_feature_metadata_value_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "fix_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(ValueError("fail")))
    resp = client.post("/fix_feature_metadata", json={
...
def test_fix_feature_metadata_internal_error(monkeypatch):
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "fix_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/fix_feature_metadata", json={
        "feature_name": "main:fixfeaturefail:v1",
        "fixed_by": "dev",
        "user_role": "developer",
...
def test_different_entities_different_values():
    ...
    timestamp = 1234567890  # Fixed timestamp for deterministic output
...
def test_lifespan_shutdown_log_explicit():
    # Covers line 111: logger.info("Shutting down feature metadata service")
...
def test_create_feature_metadata_invalid_user_role_branch(monkeypatch):
    # Covers line 151: "Invalid user role" in str(e)
    from app.main import feature_service
    ...
    resp = client.post("/create_feature_metadata", json={
        "feature_name": "main:badrole_explicit:v1",
        "feature_type": "batch",
        "feature_data_type": "float",
        "query": "SELECT 1",
        "description": "desc",
...
def test_create_feature_metadata_cannot_perform_action_branch(monkeypatch):
    # Covers line 151: "cannot perform action" in str(e)
    from app.main import feature_service
    def raise_cannot_perform(*a, **k):
        raise ValueError("User role developer cannot perform action create")
    monkeypatch.setattr(feature_service, "create_feature_metadata", raise_cannot_perform)
    resp = client.post("/create_feature_metadata", json={
        "feature_name": "main:badrole_cannot:v1",
        "feature_type": "batch",
        "feature_data_type": "float",
        "query": "SELECT 1",
        "description": "desc",
...
def test_create_feature_metadata_already_exists_branch_explicit(monkeypatch):
    # Covers line 153: "already exists" in str(e)
    from app.main import feature_service
    def raise_already_exists(*a, **k):
        raise ValueError("Feature already exists")
    monkeypatch.setattr(feature_service, "create_feature_metadata", raise_already_exists)
    resp = client.post("/create_feature_metadata", json={
...
def test_create_feature_metadata_validation_errors_branch_explicit(monkeypatch):
    # Covers line 155: "Validation errors" in str(e)
    from app.main import feature_service
    def raise_validation_error(*a, **k):
        raise ValueError("Validation errors: feature_name: Invalid feature name format")
    monkeypatch.setattr(feature_service, "create_feature_metadata", raise_validation_error)
    resp = client.post("/create_feature_metadata", json={
...
def test_approve_feature_metadata_generic_error_explicit(monkeypatch):
    # Covers line 247: logger.error(f"Unexpected error approving feature: {e}")
    from app.main import feature_service
    monkeypatch.setattr(feature_service, "approve_feature_metadata", lambda *a, **k: (_ for _ in ()).throw(Exception("fail")))
    resp = client.post("/approve_feature_metadata", json={
