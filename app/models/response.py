...
class BaseResponse(BaseModel):
    """Base response model."""
    timestamp: int = Field(default_factory=get_current_timestamp, description="Response timestamp")
...
class AllMetadataResponse(BaseResponse):
    ...
    success: bool = Field(True, description="Success indicator")
    metadata: dict[str, FeatureMetadata] | list[FeatureMetadata] = Field(..., description="All feature metadata")
...
class FilteredMetadataResponse(BaseResponse):
    ...
    success: bool = Field(True, description="Success indicator")
    metadata: list[FeatureMetadata] = Field(..., description="Filtered feature metadata")
...
class FixFeatureResponse(WorkflowResponse):
    ...
    message: str = Field("Feature fixed and reset to DRAFT", description="Success message")
...
class FeatureStatsResponse(BaseResponse):
    ...
    features_by_status: dict[str, int] = Field(..., description="Feature count by status")
    features_by_type: dict[str, int] = Field(..., description="Feature count by type")
    features_by_data_type: dict[str, int] = Field(..., description="Feature count by data type")
    recent_activity: list[dict[str, Any]] = Field(default_factory=list, description="Recent feature activity")
...
class WorkflowStatusResponse(BaseResponse):
    ...
    allowed_transitions: list[str] = Field(..., description="Allowed status transitions")
...
class CacheStatsResponse(BaseResponse):
    ...
    cache_entries: dict[str, Any] = Field(default_factory=dict, description="Cache entries info")
...
class RestoreResponse(BaseResponse):
    ...
    validation_results: dict[str, Any] = Field(default_factory=dict, description="Post-restore validation")
